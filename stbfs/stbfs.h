...
extern int get_absolute_path(struct dentry *dentry, char *file_path);
extern void* get_enc_key(struct super_block *sb);
extern int generate_hash(const u8 *input, unsigned int hash_length, u8 *output);
extern int read_write(struct file *input, struct file *output, 
				void *key, unsigned int flag);
extern int read_preamble(void *buf, struct file *input);
extern int is_owner (struct dentry *dentry, char *name);
